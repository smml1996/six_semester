...
RTree::RTree(Node * madreDeLasMadres, int &m,int &M){
  this->m = m;
  this->M = M;
  this->nodoMadre = madreDeLasMadres;
}
...
pair<Node*, Node*> RTree::splitNode(Node *n){
  ...
  for(int i =0; i < n->hijas.size(); i++){
    ...
    if(i != groups.first && i != groups.second){
  ...
  while(!elements.empty()){
      ...
      }else if (group1->hijas.size() + elements.size()<=M){
...
void RTree::insert(Node *n){
    ...
    if(!leaf->hasSpace()){
      ...
      ll = splitNode(leaf);
      leaf->mother->hijas.erase(leaf->mother->hijas.begin(),leaf->mother->hijas.begin() +  leaf->id);
      leaf->mother->hijas.push_back(ll.first);
      leaf->mother->hijas.push_back(ll.second);
      adjustTree(ll.first);
